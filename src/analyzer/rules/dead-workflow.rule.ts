 ...
 * Finds workflows that have no incoming edges in the dependency graph
 * (i.e. no UI element triggers them, and they are not custom events referenced elsewhere).
 */
...
export const deadWorkflowRule: BubbleRule = {
  ...
  name: 'Dead Workflow',
  description:
    'Detects workflows that are never triggered — no UI element references them and they have no system trigger.',
  category: 'dead-code',
  ...
  check({ app, graph, config }: RuleContext): Finding[] {
    ...
    // Collect all page and reusable element workflows
    const allWorkflows = [
      ...app.pages.flatMap((p) =>
        p.workflows.map((wf) => ({ wf, parentName: p.name })),
      ),
      ...app.reusableElements.flatMap((re) =>
        re.workflows.map((wf) => ({ wf, parentName: re.name })),
      ),
    ];

    for (const { wf, parentName } of allWorkflows) {
      // Skip workflows with always-alive triggers
      if (ALWAYS_ALIVE_TRIGGERS.has(wf.triggerType)) continue;

      // Skip if unknown trigger type (plugin events — give benefit of doubt)
      if (!isKnownClientTrigger(wf.triggerType) && wf.triggerType !== 'CustomEvent') {
        continue;
      }
      ...
      if (incomingCount === 0) {
        // For ButtonClicked workflows, check if trigger element exists
        ...
        findings.push({
          ...
          message,
          location: {
            type: wf.isCustomEvent ? 'workflow' : 'workflow',
            id: wf.id,
            name: wf.name,
          ...
          metadata: {
            ...
            triggerElementId: wf.triggerElementId,
          },
        });
      }
    }
